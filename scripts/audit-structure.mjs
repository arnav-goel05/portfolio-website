...
const trackedArtifacts = execFileSync('git', ['ls-files', 'outputs', '.qa', 'tmp'], { encoding: 'utf8' }).trim()
const walk = (directory) => readdirSync(directory).flatMap((name) => {
  const path = join(directory, name)
  return statSync(path).isDirectory() ? walk(path) : [path]
})
const sourceModules = walk('src').filter((path) => ['.ts', '.tsx'].includes(extname(path)))
...
assert(!/\.work-card:nth-child/.test(portfolioStyles), 'Project layout must not depend on card indices.')
assert(appStyles.every((line) => line.startsWith('@import ')), 'src/App.css must contain imports only.')
assert(!maintainedDocs.includes('/Users/'), 'Maintained documentation must not contain local user paths.')
assert(!trackedArtifacts, 'Generated QA/output or tmp files must not be tracked.')
assert(Object.keys(packageJson.dependencies).every((name) => ['react', 'react-dom'].includes(name)), 'Unexpected direct runtime dependency found.')
